...
namespace DLAPI
{
    ...
    public static class DalFactory
    {
        ...
        public static IDal GetDal()
        {
            ...
            {
                throw new DalConfingException($"Failed loading {dlPackageName}.dll", ex);
            }
            ...
            { // If the type is not found - the implementation is not correct - it looks like the class name is wrong...
                throw new DalConfingException($"Class not found due to a wrong namespace or/and class name: {dlPackageName}:{dlNameSpace}.{dlClass}", ex);
            }
            ...
            try
            {
                ...
                if (dal == null)
                    throw new DalConfingException($"Class {dlNameSpace}.{dlClass} instance is not initialized");
                // now it looks like we have appropriate dal implementation instance :-)
                return dal;
            }
            catch (NullReferenceException ex)
            {
                throw new DalConfingException($"Class {dlNameSpace}.{dlClass} is not a singleton", ex);
            }
